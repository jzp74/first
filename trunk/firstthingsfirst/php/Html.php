 ...
 */
function check_permissions ($action, $js_function_call_str, $error_element, $error_position)
{
    global $logging;
    global $user;
 ...
 * @return xajaxResponse every xajax registered function needs to return this object
 */
function check_list_permissions ($action, $list_title, $js_function_call_str, $error_element, $error_position)
{
    global $logging;
    global $user;
    global $user_list_permissions;
 ...
 */
function get_href ($action, $error_element, $error_position, $list_title, $func_str, $name_str, $icon_name)
{
    ...
    else
    {
        if ($list_title == HTML_EMPTY_LIST_TITLE)
        {
            ...
            $onclick_str .= "'".$error_element."', '".$error_position."')";
        }
        else
        {
 ...
 * @return string html containing button
 */
function get_href_confirm ($action, $error_element, $error_position, $list_title, $func_str, $confirm_str, $name_str, $icon_name)
{
    if ($action == HTML_NO_ACTION)
        $onclick_str = "onclick=\"".$func_str;
    else
 ...
 * @return string html containing button
 */
function get_query_href ($action, $error_element, $error_position, $list_title, $query_str, $name_str, $icon_name)
{
    if ($action == HTML_NO_ACTION)
        $onclick_str = "onclick=\"".$func_str;
    else
    {
        if ($list_title == HTML_EMPTY_LIST_TITLE)
        {
            $onclick_str = "onclick=\"xajax_check_permissions('".$action."', 'window.location.assign(";
            $onclick_str .= "%27index.php?".$query_str."%27)', '".$error_element."', '".$error_position."')";
        }
        else
        {
            $onclick_str = "onclick=\"xajax_check_list_permissions('".$action."', '".$list_title."', 'window.location.assign(";
            $onclick_str .= "%27index.php?".$query_str."%27)', '".$error_element."', '".$error_position."')";
        }
    }
...
function get_page_navigation ($page_type)
{
    ...
    if ($page_type != PAGE_TYPE_LOGIN)
    {
        ...
        if ($page_type != PAGE_TYPE_PORTAL)
        {
            ...
            $html_str .= "                    ".get_query_href(ACTION_GET_PORTAL_PAGE, "tab_portal_id", "below", HTML_EMPTY_LIST_TITLE, "action=".ACTION_GET_PORTAL_PAGE, translate("BUTTON_PORTAL"), "")."\n";
        ...
        if ($page_type != PAGE_TYPE_LISTBUILDER)
        {
            ...
            $html_str .= "                    ".get_query_href(ACTION_GET_LISTBUILDER_PAGE, "tab_listbuilder_id", "below", HTML_EMPTY_LIST_TITLE, "action=".ACTION_GET_LISTBUILDER_PAGE, translate("BUTTON_CREATE_NEW_LIST"), "")."\n";
            $html_str .= "                    <div class=\"tab_right\"></div>\n";
            $html_str .= "                </div>\n";
        }
        else
        ...
        else if (strlen($user->get_current_list_name()) > 0)
        {
            ...
            $html_str .= "                    ".get_query_href(ACTION_GET_LIST_PAGE, "tab_list_id", "below", $user->get_current_list_name(), "action=".ACTION_GET_LIST_PAGE."&list=".$user->get_current_list_name(), translate("BUTTON_LIST"), "")."\n";
            $html_str .= "                    <div class=\"tab_right\"></div>\n";
            $html_str .= "                </div>\n";
        }
        ...
        if ($page_type == PAGE_TYPE_LIST)
        {
            ...
            $html_str .= "                    ".get_query_href(ACTION_GET_USERLISTTABLEPERMISSIONS_PAGE, "tab_list_table_permissions_id", "below", $user->get_current_list_name(), "action=".ACTION_GET_USERLISTTABLEPERMISSIONS_PAGE, translate("BUTTON_USERLISTTABLEPERMISSIONS"), "")."\n";
            $html_str .= "                    <div class=\"tab_right\"></div>\n";
            $html_str .= "                </div>\n";
        }
        ...
        if ($user->is_login() && $user->get_is_admin())
        {
            ...
            if ($page_type != PAGE_TYPE_USER_ADMIN)
            {
                ...
                $html_str .= "                    ".get_query_href(ACTION_GET_USER_ADMIN_PAGE, "tab_user_admin_id", "below", HTML_EMPTY_LIST_TITLE, "action=".ACTION_GET_USER_ADMIN_PAGE, translate("BUTTON_USER_ADMINISTRATION"), "")."\n";
                $html_str .= "                    <div class=\"tab_right\"></div>\n";
                $html_str .= "                </div>\n";
            }
            else
